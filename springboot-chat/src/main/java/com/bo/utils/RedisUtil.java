...
import com.bo.redis.RedisPrefixKey;
...
public final class RedisUtil {
     ...
     */
    @SuppressWarnings("unchecked")
    public void del(RedisPrefixKey prefix,String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                redisTemplate.delete(prefix.getPrefix().concat(key[0]));
            } else {
                for (int i = 0; i < key.length; i++) {
                    key[i] = prefix.getPrefix().concat(key[i]);
                }
                redisTemplate.delete(CollectionUtils.arrayToList(key));
            }
        }
    ...
    public boolean set(RedisPrefixKey prefix, String key, Object value) {
        try {
            ...
            if (time > 0){
                ...
                redisTemplate.opsForValue().set(prefix.getPrefix().concat(key), value);
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
